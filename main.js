document.addEventListener('DOMContentLoaded', () => {
  ...
  function generateQuestion() {
    ...
    const isAddition = Math.random() > 0.5; // 50% chance of addition or subtraction

    if (isAddition) {
      currentQuestion = { num1, num2, answer: num1 + num2, operator: '+' };
    } else {
      // Ensure subtraction doesn’t result in negative answers
      const [larger, smaller] = num1 >= num2 ? [num1, num2] : [num2, num1];
      currentQuestion = { num1: larger, num2: smaller, answer: larger - smaller, operator: '-' };
    }

    questionElement.textContent = `What is ${currentQuestion.num1} ${currentQuestion.operator} ${currentQuestion.num2}?`;
  }
